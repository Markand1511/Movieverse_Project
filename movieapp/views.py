...
def actor_details(request, person_id):
    ...
    # Fallback when TMDB is slow/unreachable — use local cast data
    local_roles = list(
        Cast.objects.filter(tmdb_person_id=person_id).select_related("movie")
    )
    if local_roles:
        actor = local_roles[0]
        known_movies = [
            {
                ...
                "poster_path": None,
                "local_poster": role.movie.poster.url if role.movie.poster else "",
            }
            for role in local_roles
        ]
        character_bits = [
            f"{role.character} ({role.movie.title})"
            for role in local_roles
            if role.character
        ]
        biography = (
            f"{actor.name} is part of the MovieVerse cast."
            + (f" Known for: {', '.join(character_bits)}." if character_bits else "")
        )
        return render(request, "actor_details.html", {
            "name": actor.name,
            "biography": biography,
    ...
    messages.error(
        request,
        "Could not load actor details. Please try again in a moment.",
    )
    return redirect("homepage")
